...
export function roomFromPathname(pathname) {
    try {
        ...
        return /^document-[0-9a-f-]{36}$/i.test(room) && !room.includes('/')
            ? room.toLowerCase()
            : null
    } catch {
        return null
...
export function verifyToken(token, secret, now = Math.floor(Date.now() / 1000)) {
    ...
    try {
        ...
        const secretBytes = secret.startsWith('base64:')
...
export function resolveClientIp(
    ...
    const fallback = typeof remoteAddress === 'string' && remoteAddress !== ''
        ? remoteAddress
        : 'unknown'
...
function isTrustedProxy(remoteAddress, trustedProxies) {
    ...
    return remoteAddress.startsWith('::ffff:')
        && trustedProxies.has(remoteAddress.slice('::ffff:'.length))
}
